...
func GetCertificateSchema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"default_notification_setting": {
			Type:        schema.TypeSet,
			Computed:    true,
			Description: "The default notification configuration that is applicable to all certificates that have not been explicitly configured. This configuration determines the conditions under which notifications will be sent and to whom they will be sent.",
			Elem: &schema.Resource{
				Schema: map[string]*schema.Schema{
					"notification_type": {
						Type:     schema.TypeString,
